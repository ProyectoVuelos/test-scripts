...
if not API_KEY:
    logging.warning("PROD_FR24_API_KEY no está definida en .env. No es crítica para la simulación.")
...
def generate_simulated_positions(num_points=50, start_time=None, day_of_interest=None):
    ...
    if day_of_interest:
        ...
        if start_time < day_start_ts or start_time >= day_end_ts:
            start_time = day_start_ts + random.randint(0, 23 * 3600) # Algún punto aleatorio dentro del día
    ...
    for i in range(num_points):
        ...
        lat = start_lat + (end_lat - start_lat) * interp_factor + random.uniform(-0.1, 0.1) # Pequeña variación
        lon = start_lon + (end_lon - start_lon) * interp_factor + random.uniform(-0.1, 0.1)
        ...
        if i < num_points * 0.15: # Ascenso (primer 15%)
            ...
            alt = 35000 - (35000 - 500) * ((i - num_points * 0.85) / (num_points * 0.15))
...
def generate_simulated_summary(fr24_id, callsign_or_flight):
    ...
    if callsign_or_flight and len(callsign_or_flight) > 2 and callsign_or_flight[:2].isalpha():
    ...
    return {
        ...
        "aircraft": {
            ...
            "seats": FUEL_PROFILES.get(model, FUEL_PROFILES['default']).get('seats', 150) # Usar los asientos del perfil
        },
        ...
        "actual_sch_time_utc": datetime.utcnow().isoformat(timespec='seconds') + "Z" # Tiempo simulado
    }
...
def collect_flight_ids_for_day_simulated(day_start: datetime, interval_minutes: int):
    ...
    logging.info(f"SIMULADO: Comenzando la recolección de IDs, callsigns/flight numbers y posiciones para el día: {day_start.strftime('%Y-%m-%d')}")
    ...
    for i in range(num_simulated_flights):
        ...
        start_ts = int(day_start.timestamp()) + random.randint(0, (24 * 60 * 60) - 3600) # Vuelo empieza aleatoriamente dentro del día
        simulated_flights_for_day[fr24_id] = {
            'callsign_or_flight': callsign,
            'full_positions': generate_simulated_positions(num_points=random.randint(30, 80), start_time=start_ts, day_of_interest=day_start)
        }

    for i in range(iterations):
        current_snapshot_time_ts = int((day_start + i * interval).timestamp())
        timestamp_utc_dt = datetime.fromtimestamp(current_snapshot_time_ts, timezone.utc)
        logging.info(f"SIMULADO: Generando instantánea simulada en {timestamp_utc_dt.strftime('%Y-%m-%d %H:%M:%S UTC')}")
        ...
        for fr24_id, flight_data in simulated_flights_for_day.items():
            ...
            valid_positions_for_day = [
                p for p in full_positions 
                if day_start.timestamp() <= p["timestamp"] < (day_start + timedelta(days=1)).timestamp()
            ]
            ...
            if closest_point and min_time_diff <= interval_minutes * 60: # Dentro del intervalo de la instantánea
                flights_in_snapshot.append({
                    ...
                    "timestamp": datetime.fromtimestamp(closest_point["timestamp"], timezone.utc).isoformat(timespec='seconds') + "Z"
                })
        ...
        logging.info(f"SIMULADO: Encontrados {len(flights_in_snapshot)} IDs de vuelo en la instantánea simulada.")
    ...
    logging.info(f"SIMULADO: Se recolectaron un total de {total_unique_fr24_ids} IDs únicos y se acumularon {total_accumulated_points} puntos de posición.")
    
    all_flight_info_filtered = {
        fr24_id: data for fr24_id, data in all_flight_info.items() if data['callsign_or_flight']
    }
    logging.info(f"SIMULADO: Después de filtrar, {len(all_flight_info_filtered)} vuelos tienen callsign/flight y serán considerados para resumen.")
...
def fetch_summaries_from_ids_simulated(flight_info_map, day_start, day_end):
    ...
    all_raw_summary_responses = [] # Vacío en simulación, pero se mantiene la estructura

    fr24_ids_with_callsigns = list(flight_info_map.keys())

    if not fr24_ids_with_callsigns:
        logging.info("SIMULADO: No hay IDs de vuelo con callsigns/flight numbers para obtener resúmenes.")
        return [], []

    logging.info(f"SIMULADO: Comenzando a generar resúmenes simulados para {len(fr24_ids_with_callsigns)} vuelos.")
    ...
    logging.info(f"SIMULADO: Finalizada la generación de resúmenes. Total: {len(all_processed_summaries)}.")
...
def process_day(day_start: datetime):
    ...
    accumulated_flight_data = collect_flight_ids_for_day_simulated(day_start, INTERVAL_MINUTES)
    ...
    summaries, all_raw_summary_responses = fetch_summaries_from_ids_simulated(accumulated_flight_data, day_start, day_end) 
    ...
    if all_raw_summary_responses:
        ...
        logging.info(f"SIMULADO: No se obtuvieron respuestas raw de resúmenes (simuladas) para el día {date_str}. No se guardará el archivo consolidado.")
    ...
    try:
        ...
        logging.info(f"Resúmenes de vuelos combinados (simulados) guardados en: {summary_file_path}")
    except IOError as e:
    ...
    for fid, flight_data in accumulated_flight_data.items():
        ...
        if not s:
            logging.debug(f"ℹ️ No se encontró resumen detallado (simulado) para el vuelo FR24 ID: {fid} (Callsign: {callsign_or_flight}). Se procesará con datos parciales.")
        ...
        try:
            ...
            logging.debug(f"Puntos de posición detallados (simulados) para el vuelo {fid} guardados en: {flight_detail_file_path}")
        except IOError as e:
    ...
    try:
        ...
        logging.info(f"Datos de vuelos procesados (simulados) guardados en: {processed_file_path}")
    except IOError as e:
...
if __name__ == "__main__":
    ...
    logging.info("Iniciando el script de recolección y procesamiento de datos de FlightRadar24 (Modo Simulación).")
    for day_offset in range(DAYS):
        ...
        try:
            ...
            logging.critical(f"❌ Error crítico al procesar el día {day.strftime('%Y-%m-%d')} (simulado): {e}", exc_info=True)
    logging.info("Procesamiento de datos de FlightRadar24 finalizado (Modo Simulación).")